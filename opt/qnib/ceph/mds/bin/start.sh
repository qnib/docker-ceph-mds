...
#   CEPH_MDS_NAME - (name of metadata server)
...
# Usage:
#   docker run -e CEPH_MDS_NAME=mymds ceph/mds
...
if [ ! -n "$CEPH_MDS_NAME" ]; then
   echo "ERROR- CEPH_MDS_NAME must be defined as the name of the metadata server"
   exit 1
fi
...
if [ ! -e /var/lib/ceph/mds/ceph-${CEPH_MDS_NAME}/keyring ]; then

   mkdir -p /var/lib/ceph/mds/ceph-${CEPH_MDS_NAME}

   # See if we need to generate a key for the MDS
   if [ -e /etc/ceph/ceph.mds.keyring ]; then
      cp /etc/ceph/ceph.mds.keyring /var/lib/ceph/mds/ceph-${CEPH_MDS_NAME}/keyring
   else
      ...
      ceph auth get-or-create mds.${CEPH_MDS_NAME} mds 'allow' osd 'allow *' mon 'allow profile mds' > /var/lib/ceph/mds/ceph-${CEPH_MDS_NAME}/keyring
   fi
...
/usr/bin/ceph-mds -d -i ${CEPH_MDS_NAME}
